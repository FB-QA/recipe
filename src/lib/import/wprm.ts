 ...
 * keeps the flat JSON-LD list). Groups with no parsed ingredients are dropped; an
 * unnamed leading group is preserved (`name: null`).
 */
export function parseWprmIngredientGroups(html: string): WprmIngredientGroup[] | null {
  ...
  type Token = { pos: number; kind: "group" | "ing"; text: string };
  const tokens: Token[] = [];

  // Ingredient-group headings (specific class → excludes instruction groups).
  for (const m of html.matchAll(/wprm-recipe-ingredient-group-name[^>]*>([^<]*)</gi)) {
    tokens.push({ pos: m.index ?? 0, kind: "group", text: decodeEntities(m[1]).replace(/\s+/g, " ").trim() });
  }
  ...
  for (const m of html.matchAll(/<li[^>]*class="[^"]*\bwprm-recipe-ingredient\b[^"]*"[^>]*>([\s\S]*?)<\/li>/gi)) {
  ...
  for (const t of tokens) {
    if (t.kind === "group") {
      current = { name: t.text || null, ingredients: [] };
      groups.push(current);
    } else {
      if (!current) {
      ...
      current.ingredients.push(t.text);
    }
  }
