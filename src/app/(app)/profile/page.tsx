  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3.5 rounded-card border border-line bg-surface p-4">
        <div className="grid h-[52px] w-[52px] place-items-center rounded-full bg-gradient-to-br from-basil to-basil-2 text-xl font-extrabold text-white">
          {initial}
        </div>
        <div>
      ...
      <div className="mt-2.5 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between rounded-[14px] border border-line bg-surface px-4 py-3 text-base font-semibold text-ink">
      ...
      <p className="mt-[18px] text-center text-2xs leading-relaxed text-ink-3">
        Kept minimal for V1.
      </p>
    </>
  );
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between rounded-[14px] border border-line bg-surface px-4 py-[15px] text-base font-semibold text-ink">
      {label}
      <span className="text-sm font-medium text-ink-3">{value}</span>
    </div>
  );
}
